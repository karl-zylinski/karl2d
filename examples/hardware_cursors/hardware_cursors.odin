...
// Kept as Maybe so destroying the gauntlet can be recorded by clearing the handle. set_cursor takes
// a Maybe(Cursor) already and nil means the OS default, so a destroyed cursor needs no special case
...
make_cursor :: proc(png: []u8, hotspot: [2]int) -> k2.Cursor {
	image := k2.load_image_from_bytes(png)
	defer k2.destroy_image(image)
	return k2.create_cursor(image, hotspot)
}

init :: proc() {
	k2.init(1280, 720, "Karl2D Hardware Cursor Example")

	gauntlet = make_cursor(#load("gauntlet.png"), {4, 6})
	pointer = make_cursor(#load("pointer.png"), {4, 5})
