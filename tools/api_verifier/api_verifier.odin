// This program takes the current odingame.doc.odin and compares it to a new one. If there is any
...
main :: proc() {
	curr_doc_file, curr_doc_file_err := os.read_entire_file("odingame.doc.odin", context.allocator)

	fmt.ensuref(curr_doc_file_err == nil, "Could not open odingame.doc.odin. Error: %v", curr_doc_file_err)

	compare_filename := "odingame.doc.compare.odin"
	...
	if !ok {
		fmt.eprintln("odingame.doc.odin is not up-to-date: You may have modified the API unknowingly. From a command-line in the `odingame` folder, please run `odin run tools/api_doc_builder` and check what lines in `odingame.doc.odin` that have changed. Make sure you are really sure about these API changes.")
		os.exit(1)
	}
}