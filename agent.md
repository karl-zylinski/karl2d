...
- Automated tests live in `tests/<name>/` and are ordinary Odin tests: `odin test tests/<name>`. They need `-define:ODIN_TEST_THREADS=1`, because Karl2D keeps its state in globals, and the `nil` render and audio backends (`-define:KARL2D_RENDER_BACKEND=nil -define:KARL2D_AUDIO_BACKEND=nil`) to run somewhere with no GPU and no sound card. The test runner counts a logged error as a failure, so a backend that cannot find its device fails the test that happened to start it. `tests/coordinate_system` is run twice by CI, once per coordinate system.
