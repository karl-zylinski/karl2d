# LLM agent instructions for Odingame
...
## Project Overview
- **Odingame** is a 2D game development library written in the Odin programming language, licensed under MIT.
...
- Odingame usually requires the latest release of Odin.
- The main entry point is `odingame.odin`, which contains the platform-independent API and core logic.
- Platform and rendering backends are implemented in separate files (e.g., `platform_windows.odin`, `render_backend_gl.odin`).
- See `odingame.doc.odin` for a full API overview.
...
- When submitting a _ready for review_ Pull Request, you must:
  ...
  5. Regenerate `odingame.doc.odin` after API changes: `odin run tools/api_doc_builder`.
...
- **Procedure signatures** that are too long should be split across lines (see `init` in `odingame.odin`).
- **Spacing:**
  - Place `:` and `=` with consistent spacing as in `odingame.odin`.
...
- **File organization:**
  ...
  - Use clear section comments as in `odingame.odin`. The format is three lines: a dash line, a centered text line, and another dash line. The dashes match the width of the text. Example:
    ```
    //-------//
    // INPUT //
    //-------//
...
## Architecture Notes
- The core API is in `odingame.odin`.
...
- **Render backend selection:** On Windows, the default backend is **D3D11**. On Linux/macOS, it's **GL**. On web, it's **WebGL**. Override with `-define:ODINGAME_RENDER_BACKEND=gl` (or `d3d11`, `webgl`, `nil`). The `(GL)` VS Code build tasks use this flag.
...
- `log/log.odin` provides the internal logging utility (`odingame_logger` package) with `debugf`, `infof`, `warnf`, `errorf`, `fatalf`.
...
- `odingame.doc.odin` is generated output and should not be edited by hand.
- Update `odingame.doc.odin` for any API changes by running `odin run tools/api_doc_builder`.
...
- Never hand-edit `odingame.doc.odin`.
