...
//     odin test tests/coordinate_system -define:KARL2D_RENDER_BACKEND=nil -define:KARL2D_Y_UP=true
//
...
draw_text_is_placed_the_same_under_a_zoomed_camera :: proc(t: ^testing.T) {
	...
	expect_bounds(t, got, 12, 99.48, 102.99, 120.48, 120.51)
}
...
// The rect_* helpers are named for the screen, so they report the same screen positions in both
// coordinate systems.
@(test)
rect_helpers_are_named_for_the_screen :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, from_top(k2.rect_top_left(r)), 100)
	testing.expect_value(t, from_top(k2.rect_top_middle(r)), 100)
	testing.expect_value(t, from_top(k2.rect_top_right(r)), 100)
	testing.expect_value(t, from_top(k2.rect_bottom_left(r)), 120)
	testing.expect_value(t, from_top(k2.rect_bottom_middle(r)), 120)
	testing.expect_value(t, from_top(k2.rect_bottom_right(r)), 120)
	testing.expect_value(t, from_top(k2.rect_middle(r)), 110)

	// X is unaffected by the coordinate system, but check it is still the corner it claims to be.
...
rect_cut_top_cuts_the_top_of_the_screen :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, from_top(k2.rect_top_left(cut)), 101)
	testing.expect_value(t, cut.h, 5)
	testing.expect_value(t, from_top(k2.rect_top_left(r)), 106)
}
...
rect_cut_bottom_cuts_the_bottom_of_the_screen :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, from_top(k2.rect_top_left(cut)), 114)
	testing.expect_value(t, cut.h, 5)
	testing.expect_value(t, from_top(k2.rect_bottom_left(r)), 114)
}
